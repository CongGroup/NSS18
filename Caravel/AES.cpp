...
size_t AES::EcbEncrypt128(const char * pIn, int iInLen, char * pOut, char * pKey)
{
	EVP_CIPHER_CTX *ctx;
	...
	//Init Encrypt
	if (1 != EVP_EncryptInit_ex(ctx, EVP_aes_128_ecb(), NULL, pKey, NULL))
	{
		//Error
		return 0;
	}
...
size_t AES::EcbDecrypt128(const char * pIn, int iInLen, char * pOut, char * pKey)
{
	EVP_CIPHER_CTX *ctx;
	...
	//Init Encrypt
	if (1 != EVP_DecryptInit_ex(ctx, EVP_aes_128_ecb(), NULL, pKey, NULL))
	{
		return 0;
	}
	int iLen, iPlainLen;
